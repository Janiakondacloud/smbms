...
$(function(){
	...
	oldpassword.on("blur",function(){
		$.ajax({
			...
			data:{method:"pwdmodify",oldpassword:oldpassword.val()},//提交过去的参数
	...
	newpassword.on("focus",function(){
		validateTip(newpassword.next(),{"color":"#666666"},"* 密码长度必须是至少为6位且小于20位",false);
	}).on("blur",function(){
	...
	rnewpassword.on("focus",function(){
		...
		if(rnewpassword.val() != null && rnewpassword.val().length >= 6
			...
			validateTip(rnewpassword.next(),{"color":"green"},imgYes+"两次密码一致",true);
		}else{
